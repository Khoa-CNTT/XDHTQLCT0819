    ...
    <div class="goals-list">
      ...
      >
        ...
        <div class="goal-details">
          <span class="goal-target">Mục tiêu: {{ formatVND(goal.target) }}</span>
          <span class="goal-saved">Đã tiết kiệm: {{ formatVND(goal.save_money) }}</span>
          <span class="goal-start-date">Bắt đầu: {{ formatDate(goal.start_day) }}</span>
        ...
        <div class="progress-bar">
          ...
          >
            <span class="progress-text">{{ progress(goal.save_money, goal.target) }}%</span>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    // Get goal status class based on goal's status
    ...
    saveGoal() {
      if (this.isEditing) {
        axios.put(`/api/saving-goals/${this.currentGoal.id}`, this.currentGoal, {
        ...
        axios.post('/api/saving-goals', this.currentGoal, {
          headers: { Authorization: `Bearer ${localStorage.getItem('auth_token')}` }
        })
        .then(response => {
    ...
    deleteGoal(index) {
      ...
      axios.delete(`/api/saving-goals/${goal.id}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('auth_token')}` }
      })
      .then(response => {
    ...
    addSavingsToday(index) {
      ...
      axios.put(`/api/saving-goals/${goal.id}/add-money`, { amount: savingsToday }, {
        headers: { Authorization: `Bearer ${localStorage.getItem('auth_token')}` }
      })
      .then(response => {
