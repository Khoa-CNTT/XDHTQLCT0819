    ...
    <div class="card shadow-sm mb-4">
      <div class="card-body">
          ...
          <table class="table table-hover align-middle">
            ...
            <tbody>
              <tr v-for="(user, index) in filteredUsers" :key="user.id">
                <td>{{ index + 1 }}</td>
                <td><img :src="user.avatar ? apiImage(user.avatar) : defaultAvatar" class="avatar-sm rounded-circle" /></td>
                <td>{{ user.fullName }}</td>
                <td>{{ user.email }}</td>
                <td>{{ user.phone }}</td>
                <td>{{ user.address }}</td>
        ...
        <div class="d-flex justify-content-center mt-3">
          ...
          <button class="btn btn-outline-primary" @click="nextPage" :disabled="page * perPage >= users.length">Sau</button>
        </div>
      </div>
    </div>
